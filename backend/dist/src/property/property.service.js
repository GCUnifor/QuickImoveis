...
let PropertyService = PropertyService_1 = class PropertyService {
    ...
    constructor(prisma, firebaseService, mailService, configService) {
        this.prisma = prisma;
    ...
    async create(ownerId, dto) {
        ...
        if (!user?.is_email_verified) {
            throw new common_1.BadRequestException('É necessário verificar seu e-mail para criar imóveis.');
        }
        if (!user.creci?.trim()) {
            throw new common_1.BadRequestException('CRECI é obrigatório para criar imóveis. Cadastre seu registro no perfil.');
        }
        const property = await this.prisma.property.create({
            data: {
                ...
                status: dto.status ?? client_1.PropertyStatus.RASCUNHO,
                address: {
                    create: {
        ...
        if (property.status === client_1.PropertyStatus.DISPONIVEL) {
            this.notifyMatchingBuyers(property).catch((err) => this.logger.error('Falha ao notificar compradores', err));
        }
        return property;
    }
    ...
    async findOne(id, ownerId) {
        ...
        if (property.owner_id !== ownerId) {
            throw new common_1.ForbiddenException('Você não tem permissão para acessar este imóvel');
        }
    ...
    async addImage(propertyId, ownerId, file) {
        await this.assertOwnership(propertyId, ownerId);
        const allowedTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
        if (!allowedTypes.includes(file.mimetype)) {
            throw new common_1.BadRequestException('Tipo inválido. Use JPEG, PNG, WebP ou GIF.');
        }
        ...
        const imageUrl = await this.firebaseService.uploadFile(storagePath, file.buffer, file.mimetype);
    ...
    async reorderImages(propertyId, ownerId, imageIds) {
        await this.assertOwnership(propertyId, ownerId);
        ...
        if (invalidId || imageIds.length !== validIds.size) {
            throw new common_1.BadRequestException('Lista de IDs inválida. Todos os IDs devem pertencer ao imóvel.');
        }
        await this.prisma.$transaction(imageIds.map((id, index) => this.prisma.propertyImage.update({
            where: { id },
    ...
    async update(id, ownerId, dto) {
        ...
        const addressData = dto.address !== undefined
            ? {
                street: dto.address.street,
                number: dto.address.number,
                neighborhood: dto.address.neighborhood,
        ...
        const becameAvailable = dto.status === client_1.PropertyStatus.DISPONIVEL &&
            current.status !== client_1.PropertyStatus.DISPONIVEL;
        if (becameAvailable) {
            this.notifyMatchingBuyers(updated).catch((err) => this.logger.error('Falha ao notificar compradores', err));
        }
        return {
            ...updated,
    ...
    async notifyMatchingBuyers(property) {
        ...
        const matched = buyers.filter((buyer) => {
            ...
            const sameLocation = (propState && buyerState === propState) ||
        ...
        const location = [property.address?.neighborhood, property.address?.city, property.address?.state]
            .filter(Boolean)
            .join(', ');
        const promises = matched.map((buyer) => this.mailService.sendMail({
            to: buyer.email,
...
<body style="margin:0;padding:0;background-color:#f4f4f5;font-family:'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f5;padding:32px 16px;">
    ...
    async assertOwnership(propertyId, ownerId) {
        const property = await this.prisma.property.findUnique({
            where: { id: propertyId },
        });
        if (!property) {
        ...
        if (property.owner_id !== ownerId) {
            throw new common_1.ForbiddenException('Você não tem permissão para modificar este imóvel');
        }
        return property;
    }
};
