...
let FirebaseService = FirebaseService_1 = class FirebaseService {
    ...
    loadCredentials() {
        const credentialsJson = this.configService.get('FIREBASE_CREDENTIALS');
        if (credentialsJson) {
    ...
    propertyImagePath(propertyId, mimetype) {
        const ext = mimetype.split('/')[1] ?? 'jpeg';
        return `properties/${propertyId}/${(0, crypto_1.randomUUID)()}.${ext}`;
    }
};
